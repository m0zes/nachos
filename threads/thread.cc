...
bool
Thread::CreateFile(char *fn) {
  return(space->CreateFile(fn));
}

int
Thread::OpenReadWriteFile(char *fn) {
  return(space->OpenReadWriteFile(fn));
}

int 
Thread::ReadOpenFile(int fd, int svaddr, int size) {
  return(space->ReadOpenFile(fd, svaddr, size));
}

int 
Thread::WriteOpenFile(int fd, int svaddr, int size) {
  return(space->WriteOpenFile(fd, svaddr, size));
}
 
void
Thread::CloseOpenFile(int fd) { 
  space->CloseOpenFile(fd);
}

int 
Thread::ReadConsole(int b, int size) { 
  return(space->ReadConsole(b, size));
}
int 
Thread::WriteConsole(int b, int size) { 
  return(space->WriteConsole(b, size));
}
bool
